...
def processOneImage(specifiedRegions, boundingBoxes, timeSeries, predictedBoundingBoxes, shortTermPredictedTimeSeries):
    ...
    for box in boundingBoxes:
        for i in range(len(specifiedRegions)):
            regionCurrentCounts[i] += (box[3].item() - box[1].item()) ** 2 * computeFractionOfIntersection(specifiedRegions[i], box)
    ...
    for box in predictedBoundingBoxes:
        for i in range(len(specifiedRegions)):
            regionPredictedCounts[i] += (box[3].item() - box[1].item()) ** 2 * computeFractionOfIntersection(specifiedRegions[i], box)
